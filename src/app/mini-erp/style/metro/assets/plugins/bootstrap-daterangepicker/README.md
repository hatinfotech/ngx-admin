...
Several events are triggered on the element you attach the picker to, which you can run for:
...
While passing in a callback to the constructor is the easiest way to run for changes in the selected date range, you can also do something every time the apply button is clicked even if the selection hasn't changed:
