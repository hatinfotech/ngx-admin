...
export class PriceReportService implements ApiResource<PriceReportModel> {
  ...
  get(params: Object, success: (resources: PriceReportModel[]) => void, error: (e) => void) {
    this.apiService.get<PriceReportModel[]>('/sales/price-reports', params, success, error);
  }
  getById(id: string, success: (resources: PriceReportModel) => void, error: (e) => void) {
    this.apiService.get<PriceReportModel>('/sales/price-reports', { 'id': id}, success, error);
  }
  post(resource: PriceReportModel, success: (newResource: PriceReportModel) => void, error: (e) => void) {
    this.apiService.post<PriceReportModel>('/sales/price-reports', resource, success, error);
  }
  put(resource: PriceReportModel, success: (newResource: PriceReportModel) => void, error: (e) => void) {
