...
export class ContactService implements ApiResource<ContactModel> {
  ...
  get(params: Object, success: (resources: ContactModel[]) => void, error: (e) => void) {
    this.apiService.get<ContactModel[]>('/contact/contacts', params, success, error);
  }
  getById(id: string, success: (resources: ContactModel) => void, error: (e) => void) {
    this.apiService.get<ContactModel>('/contact/contacts', { 'id': id}, success, error);
  }
  post(resource: ContactModel, success: (newResource: ContactModel) => void, error: (e) => void) {
    this.apiService.post<ContactModel>('/contact/contacts', resource, success, error);
  }
  put(resource: ContactModel, success: (newResource: ContactModel) => void, error: (e) => void) {
