...
export class UnitService implements ApiResource<UnitModel> {
  ...
  get(params: Object, success: (resources: UnitModel[]) => void, error: (e: HttpErrorResponse) => void) {
    this.apiService.get<UnitModel[]>('/admin-product/units', params, success, error);
  }
  getById(id: string, success: (resources: UnitModel) => void, error: (e: HttpErrorResponse) => void) {
    this.apiService.get<UnitModel>('/admin-product/units', { 'id': id}, success, error);
  }
  post(resource: UnitModel, success: (newResource: UnitModel) => void, error: (e: HttpErrorResponse) => void) {
