...
export class DeploymentVoucherFormComponent extends DataManagerFormComponent<DeploymentVoucherModel> implements OnInit {
  ...
  select2OptionForTransportation = {
    placeholder: this.commonService.translateText('Phương tiện vận chuyển', { definition: '', action: '' }) + '...',
    allowClear: true,
    width: '100%',
    dropdownAutoWidth: true,
    minimumInputLength: 0,
  ...
  onSelectProduct(detail: FormGroup, selectedData: ProductModel, parentForm: FormGroup, detailForm?: FormGroup) {
    ...
    if (selectedData && this.commonService.getObjectId(selectedData) == 'BANLE' && detailForm) {
      this.apiService.getPromise('/accounting/reports', {
        reportSummary: true,
        Accounts: '1111',
        toDate: this.commonService.getEndOfDate(parentForm.get('DeploymentDate')?.value).toISOString(),
      }).then(rs => {
        console.log(rs);
              ...
              if (index < 0) {
                ...
                if (this.commonService.getObjectId(formGroup.get('Object').value)) {
                  if (this.commonService.getObjectId(refVoucher.Object, 'Code') != this.commonService.getObjectId(formGroup.get('Object').value)) {
                    this.commonService.showToast(this.commonService.translateText('Khách hàng trong phiếu bán hàng không giống với phiếu triển khai'), this.commonService.translateText('Common.warning'), { status: 'warning' });
                    continue;
                  }
                } else {
          ...
          if (type === 'PURCHASE') {
            for (let i = 0; i < chooseItems.length; i++) {
              ...
              if (index < 0) {
                ...
                if (this.commonService.getObjectId(formGroup.get('Object').value)) {
                  if (this.commonService.getObjectId(refVoucher.Object, 'Code') != this.commonService.getObjectId(formGroup.get('Object').value)) {
                    this.commonService.showToast(this.commonService.translateText('Nhà cung cấp trong phiếu mua hàng không giống với phiếu triên khai'), this.commonService.translateText('Common.warning'), { status: 'warning' });
                    continue;
                  }
                } else {
                  delete refVoucher.Id;
          ...
          if (type === 'PURCHASEORDER') {
            for (let i = 0; i < chooseItems.length; i++) {
              ...
              if (index < 0) {
                ...
                if (this.commonService.getObjectId(formGroup.get('Object').value)) {
                  if (this.commonService.getObjectId(refVoucher.Object, 'Code') != this.commonService.getObjectId(formGroup.get('Object').value)) {
                    this.commonService.showToast(this.commonService.translateText('Nhà cung cấp trong phiếu đặt mua hàng không giống với phiếu triên khai'), this.commonService.translateText('Common.warning'), { status: 'warning' });
                    continue;
                  }
                } else {
                  delete refVoucher.Id;
