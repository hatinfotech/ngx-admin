...
@Component({
  selector: 'ngx-mkt-promotion-program-list',
  templateUrl: './promotion-program-list.component.html',
  styleUrls: ['./promotion-program-list.component.scss'],
  providers: [CurrencyPipe, DatePipe],
})
export class MktPromotionProgramListComponent extends AgGridDataManagerListComponent<Model, MktPromotionProgramFormComponent> implements OnInit {

  componentName: string = 'MktPromotionProgramListComponent';
  formPath = '';
  ...
  constructor(
    ...
    public ref: NbDialogRef<MktPromotionProgramListComponent>,
  ...
  async init() {
    return super.init().then(async state => {
      // Add page choosed
      // this.marketingService.pageList$.pipe(filter(f => f && f.length > 0), take(1)).toPromise().then(pageList => {
      ...
      //       }
      //     },
      //     asyncValue: this.marketingService.currentpage$,
      ...
      const processingMap = {
        ...
        "APPROVED": {
          ...
          nextStates: [
            ...
            { ...this.closeState, status: 'success' },
          ],
        },
        "CLOSE": {
          ...this.closeState,
      ...
      this.columnDefs = this.configSetting([
        ...
        },
        {
          ...agMakeStateColDef(this.cms, processingMap, (data) => {
            ...
            if (stateId == 'NOTJUSTAPPROVED' || stateId == 'CLOSE') {
              this.cms.showDialog('Phê duyệt chương trình khuyến mại', 'Bạn có muốn phê duyệt cho chương trình khuyến mại "' + data.Title + '"', [
                {
                  label: 'Đóng',
                  status: 'basic',
                  outline: true,
              ...
              this.cms.showDialog('Khởi chạy chiến dịch chiết khấu nâng cao', 'Bạn có muốn đóng chương trinh khuyến mại "' + data.Title + '"', [
                {
                  label: 'Đóng',
                  status: 'basic',
                  outline: true,
                ...
                },
                {
                  ...
                  outline: true,
                  action: () => {
                    this.apiService.putPromise(this.apiPath, { changeState: 'CLOSE' }, [{ Code: data.Code }]).then(rs => {
                      this.refresh();
                      this.cms.toastService.show(data.Title, 'Đã đóng chương trinh khuyến mại !', { status: 'success' });
                    });
                  }
                },
              ]);
  ...
  prepareApiParams(params: any, getRowParams: IGetRowsParams) {
    // params['page'] = this.marketingService?.currentpage$?.value;
    return params;
  }
  ...
  //   if (page !== null) {
  //     // this.marketingService.currentpage$.next(this.cms.getObjectId(page));
