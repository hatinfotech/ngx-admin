...
@Component({
  ...
  styleUrls: ['./member-card-print.component.css'],
})
export class MktMemberCardPrintComponent extends DataManagerPrintComponent<any> implements OnInit {

  /** Component name */
  componentName = 'MktMemberCardPrintComponent';
  title: string = '';
  env = environment;
  ...
  ngOnInit() {
    this.restrict();
    super.ngOnInit();
    this.cms.systemConfigs$.pipe(filter(f => !!f)).subscribe(registerInfo => {
      this.registerInfo = registerInfo.LICENSE_INFO.register;
      this.registerInfo.voucherInfo = this.registerInfo.voucherInfo.replace(/\\n/g, '<br>');
    });
