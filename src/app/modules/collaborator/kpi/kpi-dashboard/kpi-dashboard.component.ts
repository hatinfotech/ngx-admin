...
@Component({
  selector: 'ngx-collaborator-kpi-dashboard',
  templateUrl: './kpi-dashboard.component.html',
  styleUrls: ['./kpi-dashboard.component.scss'],
  providers: [CurrencyPipe, DecimalPipe]
})
export class CollaboratorKpiDashboardComponent extends BaseComponent {

  componentName = 'CollaboratorKpiDashboardComponent';
  ...
  topListChoose = new FormControl('PUBLISHERSHPPORTERS');
  ...
  select2OptionForTopTab = {
    ...
    data: [
      { id: 'PUBLISHERSHPPORTERS', text: 'DS Nhân viên chăm sóc CTV' },
  ...
  onPublisherSupporterClickClick(publisher: any) {
    this.formItem.get('PublisherSupporters').setValue([publisher]);
  }
  ...
  async refresh() {
    ...
    promiseAll.push(new Promise((resolve, reject) => {
      ...
      //   console.log(rs);
      // });
      this.apiService.getPromise<any>('/collaborator/publisher-supporters', { limit: 10 }).then(rs => {
        this.publisherSupporterList = rs;
        console.log(rs);
      });
    ...
    promiseAll.push(new Promise(async (resolve, reject) => {
      ...
      timeline = [...new Set([
        ...
        ...lineData['numOfOrderLine7Data'].map(item => item['Timeline']),
      ].sort())];
      labels = [];
      ...
      mergeData = timeline.map(t => {
        ...
        const point7 = lineData['numOfOrderLine7Data'].find(f => f.Timeline == t);
        labels.push(
          point1?.Label
          || point2?.Label
          || point3?.Label
      ...
      this.charts['revenue'] = {
        ...
        options: {
          ...
          tooltips: {
            callbacks: {
              label: (tooltipItem, data) => {
                let sum = data.datasets.reduce((previous, dataset) => { return dataset.part ? (previous + parseFloat(dataset.data[tooltipItem.index])) : previous; }, 0);
