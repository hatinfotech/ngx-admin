...
export class CollaboratorRebuyStrategyListComponent extends ServerDataManagerListComponent<CollaboratorRebuyStrategyModel> implements OnInit {

  componentName: string = 'CollaboratorRebuyStrategyListComponent';
  formPath = '';
  ...
  constructor(
    ...
    public ref: NbDialogRef<CollaboratorRebuyStrategyListComponent>,
  ...
  reuseDialog = true;
  static _dialog: NbDialogRef<CollaboratorRebuyStrategyListComponent>;
                  ...
                  },
                  {
                    ...
                    action: () => {
                      this.apiService.putPromise(this.apiPath, { changeState: 'APPROVED' }, [{ Code: instance.rowData.Code }]).then(rs => {
                        this.refresh();
                        this.commonService.showToast(instance.rowData.Title, 'Đã phê duyệt chiến dịch chiết khấu tái mua !', { status: 'success' });
                      });
                    }
                  }
                ]);
                  ...
                  },
                  {
                    ...
                    action: () => {
                      this.apiService.putPromise(this.apiPath, { changeState: 'COMPLETE' }, [{ Code: instance.rowData.Code }]).then(rs => {
                        this.refresh();
                        this.commonService.showToast(instance.rowData.Title, 'Đã hoàn tất chiến dịch chiết khấu tái mua !', { status: 'success' });
                      });
                    }
                  },
                  {
                    ...
                    action: () => {
                      this.apiService.putPromise(this.apiPath, { changeState: 'UNRECORDED' }, [{ Code: instance.rowData.Code }]).then(rs => {
                        this.refresh();
                        this.commonService.showToast(instance.rowData.Title, 'Đã hủy chiến dịch chiết khấu tái mua !', { status: 'success' });
                      });
                    }
                  },
                ]);
  ...
  initDataSource() {
    ...
    source.prepareData = (data: CollaboratorRebuyStrategyModel[]) => {
      // data.map((product: CollaboratorRebuyStrategyModel) => {
  ...
  getList(callback: (list: CollaboratorRebuyStrategyModel[]) => void) {
    super.getList((rs) => {
