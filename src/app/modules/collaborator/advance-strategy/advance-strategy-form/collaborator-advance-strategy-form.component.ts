...
export class CollaboratorAdvanceStrategyFormComponent extends DataManagerFormComponent<CollaboratorAddonStrategyModel> implements OnInit {
      ...
      },
      {
        ...
        cellRendererParams: {
          ...
          clicked: (params: { node: RowNode, data: CollaboratorAdvanceStrategyPublisherModel, api: GridApi } & { [key: string]: any }) => {
            ...
            // Setting for product
            this.commonService.openDialog(CollaboratorAdvanceStrategyPublisherFormComponent, {
              context: {
                data: [
                  params.data,
                ],
                onDialogSave(newData) {
                  console.log(newData);
                  let currentNode: RowNode = $this.gridApi.getRowNode($this.commonService.getObjectId(params.data.Publisher));
  ...
  public getRowNodeId = (item: CollaboratorAdvanceStrategyPublisherModel) => {
    return this.commonService.getObjectId(item.Publisher);
  }
  ...
  loadList(callback?: (list: CollaboratorAdvanceStrategyPublisherModel[]) => void) {
    if (this.gridApi) {
      let products: CollaboratorAdvanceStrategyPublisherModel[] = (this.array.controls[0].get('Publishers').value || []).map((detail: CollaboratorAdvanceStrategyPublisherModel) => {
        // if (detail.Container) {
  ...
  initDataSource() {
    this.dataSource = {
      ...
      getRows: (getRowParams: IGetRowsParams) => {
        ...
        let details: CollaboratorAdvanceStrategyPublisherModel[] = (this.array.controls[0]['Publishers'] as []).slice(getRowParams.startRow, getRowParams.endRow);
        let lastRow = -1;
  ...
  addProduct(formItem: FormGroup) {
    const $this = this;
    this.commonService.openDialog(CollaboratorPublisherListComponent, {
      context: {
      ...
      context: {
        ...
        onDialogSave(newData) {
          ...
          for (const itemData of newData) {
            let currentNode: RowNode = $this.gridApi.getRowNode($this.commonService.getObjectId(itemData.Publisher));
