...
export class SaleProductListComponent extends ServerDataManagerListComponent<SalesProductModel> implements OnInit {
      ...
      columns: {
        ...
        FeaturePicture: {
          ...
          onComponentInitFunction: (instance: SmartTableThumbnailComponent) => {
            ...
            instance.click.subscribe((row: SalesProductModel) => {
        ...
        Product: {
          ...
          type: 'string',
          width: '20%',
          valuePrepareFunction: (cell, row: SalesProductModel) => {
        ...
        Name: {
          title: 'Tên theo khách hàng',
          type: 'string',
          width: '15%',
        },
        Sku: {
          title: 'Sku theo khách hàng',
          type: 'string',
          width: '5%',
        },
        TaxName: {
        ...
        ReferenceVoucher: {
          ...
          valuePrepareFunction: (cell: string, row: SalesProductModel) => {
  ...
  initDataSource() {
    ...
    source.prepareData = (data: SalesProductModel[]) => {
      data.map((product: SalesProductModel) => {
  ...
  /** Api get funciton */
  executeGet(params: any, success: (resources: SalesProductModel[]) => void, error?: (e: HttpErrorResponse) => void, complete?: (resp: SalesProductModel[] | HttpErrorResponse) => void) {
    params['includeCategories'] = true;
    super.executeGet(params, success, error, complete);
  }

  getList(callback: (list: SalesProductModel[]) => void) {
    super.getList((rs) => {
      // rs.map((product: any) => {
