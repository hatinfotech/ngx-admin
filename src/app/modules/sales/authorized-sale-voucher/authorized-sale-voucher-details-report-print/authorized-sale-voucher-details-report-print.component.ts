...
@Component({
  selector: 'ngx-authorized-sale-voucher-details-report-print',
  templateUrl: './authorized-sale-voucher-details-report-print.component.html',
  styleUrls: ['./authorized-sale-voucher-details-report-print.component.scss'],
  providers: [CurrencyPipe]
})
export class AuthorizedSaleVoucherDetailsReportPrintComponent extends DataManagerPrintComponent<CashVoucherModel> implements OnInit {

  /** Component name */
  componentName = 'AuthorizedSaleVoucherDetailsReportPrintComponent';
  title: string = 'Báo cáo chi tiết';
  ...
  constructor(
    ...
    public ref: NbDialogRef<AuthorizedSaleVoucherDetailsReportPrintComponent>,
    ...
    for (const supplier of this.suppliers) {
      promiseAll.push(this.apiService.getPromise<any[]>(this.apiPath, {
        ...
        const item = {
          ...
          ToDate: this.toDate,
          ReportDate: new Date(),
          Details: data.reduce((results, voucher, index) => {
            for (const detail of voucher.Details) {
              if (detail.Type != 'CATEGORY') {
                detail.VoucherDate = voucher.DateOfSale;
                incrementAmount += detail.Amount;
                detail.IncrementAmount = incrementAmount;
                results.push(detail);
              }
            }
            return results;
