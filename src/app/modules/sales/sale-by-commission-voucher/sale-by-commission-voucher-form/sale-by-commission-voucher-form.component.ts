...
import { SaleByCommissionVoucherModel, SaleByCommissionVoucherDetailModel, SalesMasterPriceTableDetailModel } from '../../../../models/sales.model';
...
@Component({
  selector: 'ngx-sale-by-commission-voucher-form',
  templateUrl: './sale-by-commission-voucher-form.component.html',
  styleUrls: ['./sale-by-commission-voucher-form.component.scss'],
  providers: [DatePipe]
})
export class SaleByCommissionVoucherFormComponent extends DataManagerFormComponent<SaleByCommissionVoucherModel> implements OnInit {

  componentName: string = 'SaleByCommissionVoucherFormComponent';
  idKey = 'Code';
  apiPath = '/sales/sale-by-commission-vouchers';
  baseFormUrl = '/sales/sale-by-commission-voucher/form';
  ...
  constructor(
    ...
    public ref: NbDialogRef<SaleByCommissionVoucherFormComponent>,
  ...
  async init(): Promise<boolean> {
    ...
    // if (!SaleByCommissionVoucherFormComponent._taxList) {
    // } else {
    //   this.taxList = SaleByCommissionVoucherFormComponent._taxList;
    // }
    ...
    //   return tax;
    // });
    // if (!SaleByCommissionVoucherFormComponent._unitList) {
    // } else {
    //   this.taxList = SaleByCommissionVoucherFormComponent._taxList;
    // }
    ...
    this.rsv.accountingService.accountingBusinessList$.pipe(filter(f => !!f), takeUntil(this.destroy$)).subscribe(list => {
      this.accountingBusinessList = list.filter(f => ['SALEBYCOMMISSION'].indexOf(f.Type) > -1);
    });
  ...
  /** Execute api get */
  executeGet(params: any, success: (resources: SaleByCommissionVoucherModel[]) => void, error?: (e: HttpErrorResponse) => void) {
  ...
  async formLoad(formData: SaleByCommissionVoucherModel[], formItemLoadCallback?: (index: number, newForm: FormGroup, formData: SaleByCommissionVoucherModel) => void) {
  ...
  makeNewFormGroup(data?: SaleByCommissionVoucherModel): FormGroup {
  ...
  patchFormGroupValue = (formGroup: FormGroup, data: SaleByCommissionVoucherModel) => {
  ...
  onAddFormGroup(index: number, newForm: FormGroup, formData?: SaleByCommissionVoucherModel): void {
  ...
  /** Detail Form */
  makeNewDetailFormGroup(parentFormGroup: FormGroup, data?: SaleByCommissionVoucherDetailModel): FormGroup {
    let newForm = null;
    newForm = this.formBuilder.group({
      ...
      Business: {disabled: true, value: null},
    });
      ...
      if (selectedData && !selectedData['doNotAutoFill']) {
        ...
        if (selectedData.Code) {
          ...
          this.apiService.getPromise<PriceReportModel[]>('/sales/price-reports/' + selectedData.Code, {
            ...
            if (rs && rs.length > 0) {
              const salesVoucher: SaleByCommissionVoucherModel = { ...rs[0] };
  ...
  //   const data: SaleByCommissionVoucherModel = formItem.value;
  ...
  //   });
  //   this.cms.openDialog(SaleByCommissionVoucherPrintComponent, {
  //     context: {
  //       title: 'Xem trước',
  //       data: [data],
  //       mode: 'preview',
  //       idKey: ['Code'],
  //       onSaveAndClose: (rs: SaleByCommissionVoucherModel) => {
  //         this.saveAndClose();
  //       },
  //       onSaveAndPrint: (rs: SaleByCommissionVoucherModel) => {
  //         this.save();
  //       },
  //     },
  //   });
