...
import { SaleByCommissionVoucherDetailModel, SaleByCommissionVoucherModel } from '../../../../models/sales.model';

@Component({
  selector: 'ngx-sale-by-commission-voucher-print',
  templateUrl: './sale-by-commission-voucher-print.component.html',
  styleUrls: ['./sale-by-commission-voucher-print.component.scss'],
})
export class SaleByCommissionVoucherPrintComponent extends DataManagerPrintComponent<SaleByCommissionVoucherModel> implements OnInit {

  /** Component name */
  componentName = 'SaleByCommissionVoucherPrintComponent';
  title: string = '';
  env = environment;
  ...
  constructor(
    ...
    public ref: NbDialogRef<SaleByCommissionVoucherPrintComponent>,
  ...
  async init() {
    ...
    // this.title = `SaleByCommissionVoucher_${this.identifier}` + (this.data.DateOfSale ? ('_' + this.datePipe.transform(this.data.DateOfSale, 'short')) : '');
  ...
  renderTitle(data: SaleByCommissionVoucherModel) {
  ...
  toMoney(detail: SaleByCommissionVoucherDetailModel) {
  ...
  prepareCopy(data: SaleByCommissionVoucherModel) {
    this.close();
  ...
  approvedConfirm(data: SaleByCommissionVoucherModel) {
    ...
    this.cms.showDialog(this.cms.translateText('Common.confirm'), this.cms.translateText(processMap?.confirmText, { object: this.cms.translateText('Sales.PriceReport.title', { definition: '', action: '' }) + ': `' + data.Title + '`' }), [
      ...
      },
      {
        ...
        action: () => {
          this.loading = true;
          this.apiService.putPromise<SaleByCommissionVoucherModel[]>(this.apiPath, params, [{ Code: data.Code }]).then(rs => {
            this.loading = false;
  ...
  approvedConfirmX(data: SaleByCommissionVoucherModel) {
    if (data.State === 'COMPLETE') {
      this.cms.showDialog(this.cms.translateText('Common.notice'), this.cms.translateText('Common.completedNotice', { resource: this.cms.translateText('Sales.SaleByCommissionVoucher.title', { action: '', definition: '' }) }), [
        {
    ...
    this.cms.showDialog(this.cms.translateText('Common.confirm'), this.cms.translateText(!data.State ? 'Common.approvedConfirm' : (data.State === 'APPROVE' ? 'Common.completedConfirm' : ''), { object: this.cms.translateText('Sales.SaleByCommissionVoucher.title', { definition: '', action: '' }) + ': `' + data.Title + '`' }), [
      {
        label: this.cms.translateText('Common.cancel'),
        status: 'primary',
        action: () => {
      ...
      },
      {
        ...
        action: () => {
          ...
          this.apiService.putPromise<SaleByCommissionVoucherModel[]>('/sales/sale-by-commission-vouchers', params, [{ Code: data.Code }]).then(rs => {
  ...
  async getFormData(ids: string[]) {
    return this.apiService.getPromise<SaleByCommissionVoucherModel[]>(this.apiPath, { id: ids, includeContact: true, includeObject: true, includeDetails: true, useBaseTimezone: true, includeTax: true, includeUnit: true, includeRelativeVouchers: true, includeSignature: true }).then(rs => {
      for (const item of rs) {
        const processMap = AppModule.processMaps.salesVoucher[item.State || ''];
        item.StateLabel = processMap.label;
        if (item && item.Details) {
          this.setDetailsNo(item.Details, (detail: SaleByCommissionVoucherDetailModel) => detail.Type !== 'CATEGORY');
  ...
  getItemDescription(item: SaleByCommissionVoucherModel) {
    return item?.Description;
  }

  summaryCalculate(data: SaleByCommissionVoucherModel[]) {
    for (const i in data) {
      const item = data[i];
      item['Total'] = 0;
  ...
  receipt(item: SaleByCommissionVoucherModel) {
