...
import { SalesB2bQuotationModel, SalesB2bQuotationDetailModel } from '../../../../models/sales.model';
...
@Component({
  selector: 'ngx-b2b-quotation-print',
  templateUrl: './sales-b2b-quotation-print.component.html',
  styleUrls: ['./sales-b2b-quotation-print.component.scss'],
})
export class SalesB2bQuotationPrintComponent extends DataManagerPrintComponent<SalesB2bQuotationModel> implements OnInit {

  /** Component name */
  componentName = 'SalesB2bQuotationPrintComponent';
  ...
  constructor(
    ...
    public ref: NbDialogRef<SalesB2bQuotationPrintComponent>,
  ...
  // getIdentified(data: SalesB2bQuotationModel): string[] {
  ...
  renderTitle(data: SalesB2bQuotationModel) {
  ...
  toMoney(detail: SalesB2bQuotationDetailModel) {
  ...
  getTotal(data: SalesB2bQuotationModel) {
    let total = 0;
  ...
  saveAndClose(data: SalesB2bQuotationModel) {
  ...
  prepareCopy(data: SalesB2bQuotationModel) {
    this.close();
  ...
  approvedConfirm(data: SalesB2bQuotationModel, index: number) {
    ...
    this.cms.showDialog(this.cms.translateText('Common.confirm'), this.cms.translateText(this.processMapList[index]?.confirmText, { object: this.cms.translateText('Sales.PriceReport.title', { definition: '', action: '' }) + ': `' + data.Title + '`' }), [
      ...
      },
      {
        ...
        action: () => {
          this.loading = true;
          this.apiService.putPromise<SalesB2bQuotationModel[]>('/sales/price-reports', params, [{ Code: data.Code }]).then(rs => {
            this.loading = true;
  ...
  async getFormData(ids: string[]) {
    return this.apiService.getPromise<SalesB2bQuotationModel[]>(this.apiPath, { id: ids, includeContact: true, includeDetails: true, includeTax: true, includeUnit: true, includeRelativeVouchers: true }).then(rs => {
      if (rs[0] && rs[0].Details) {
        this.setDetailsNo(rs[0].Details, (detail: SalesB2bQuotationDetailModel) => detail.Type !== 'CATEGORY');
        // let total = 0;
  ...
  getItemDescription(item: SalesB2bQuotationModel) {
    return item?.Title;
  }

  summaryCalculate(data: SalesB2bQuotationModel[]) {
