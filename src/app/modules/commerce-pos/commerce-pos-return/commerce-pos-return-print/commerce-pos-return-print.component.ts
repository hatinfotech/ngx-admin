...
export class CommercePosReturnPrintComponent extends DataManagerPrintComponent<CommercePosReturnModel> implements OnInit {
  ...
  approvedConfirmX(data: CommercePosReturnModel) {
    if (data.State === 'COMPLETE') {
      this.commonService.showDialog(this.commonService.translateText('Common.notice'), this.commonService.translateText('Common.completedNotice', { resource: this.commonService.translateText('Sales.CommercePosReturn.title', { action: '', definition: '' }) }), [
        {
    ...
    this.commonService.showDialog(this.commonService.translateText('Common.confirm'), this.commonService.translateText(!data.State ? 'Common.approvedConfirm' : (data.State === 'APPROVE' ? 'Common.completedConfirm' : ''), { object: this.commonService.translateText('Sales.CommercePosReturn.title', { definition: '', action: '' }) + ': `' + data.Title + '`' }), [
      {
        label: this.commonService.translateText('Common.cancel'),
        status: 'primary',
        action: () => {
