...
@Component({
  selector: 'ngx-commerce-pos-return-form',
  templateUrl: './commerce-pos-return-form.component.html',
  styleUrls: ['./commerce-pos-return-form.component.scss'],
  providers: [DatePipe]
})
export class CommercePosReturnFormComponent extends DataManagerFormComponent<CommercePosReturnModel> implements OnInit {

  componentName: string = 'CommercePosReturnFormComponent';
  idKey = 'Code';
  apiPath = '/commerce-pos/returns';
  baseFormUrl = '/commerce-pos/return/form';
  ...
  async init(): Promise<boolean> {
    ...
    // if (!CommercePosReturnFormComponent._taxList) {
    // } else {
    ...
    //   return tax;
    // });
    // if (!CommercePosReturnFormComponent._unitList) {
    // } else {
    //   this.taxList = CommercePosReturnFormComponent._taxList;
    // }
  ...
  /** Execute api get */
  executeGet(params: any, success: (resources: CommercePosReturnModel[]) => void, error?: (e: HttpErrorResponse) => void) {
  ...
  async formLoad(formData: CommercePosReturnModel[], formItemLoadCallback?: (index: number, newForm: FormGroup, formData: CommercePosReturnModel) => void) {
  ...
  makeNewFormGroup(data?: CommercePosReturnModel): FormGroup {
    ...
    newForm.get('ObjectName').valueChanges.pipe(takeUntil(this.destroy$)).subscribe(objectName => {
      if (objectName && (!titleControl.touched || !titleControl.value) && (!titleControl.value || /^Trả hàng: /.test(titleControl.value))) {
  ...
  patchFormGroupValue = (formGroup: FormGroup, data: CommercePosReturnModel) => {
  ...
  onAddFormGroup(index: number, newForm: FormGroup, formData?: CommercePosReturnModel): void {
      ...
      if (selectedData && !selectedData['doNotAutoFill']) {
        ...
        if (selectedData.Code) {
          ...
          this.apiService.getPromise<PriceReportModel[]>('/sales/price-reports/' + selectedData.Code, {
            ...
            if (rs && rs.length > 0) {
              const salesVoucher: CommercePosReturnModel = { ...rs[0] };
  ...
  //       mode: 'preview',
  //       idKey: ['Code'],
  //       onSaveAndClose: (rs: CommercePosReturnModel) => {
  //         this.saveAndClose();
  //       },
  //       onSaveAndPrint: (rs: CommercePosReturnModel) => {
  //         this.save();
  //       },
  //     },
  //   });
              ...
              if (index < 0) {
                ...
                if (this.commonService.getObjectId(formGroup.get('Object').value)) {
                  if (this.commonService.getObjectId(refVoucher.Object, 'Code') != this.commonService.getObjectId(formGroup.get('Object').value)) {
                    this.commonService.showToast(this.commonService.translateText('Khách hàng trong phiếu mua hàng không giống với phiếu bán hàng'), this.commonService.translateText('Common.warning'), { status: 'warning' });
                    continue;
                  }
                } else {
          ...
          if (type === 'PRICEREPORT') {
            for (let i = 0; i < chooseItems.length; i++) {
              ...
              if (index < 0) {
                ...
                if (this.commonService.getObjectId(formGroup.get('Object').value)) {
                  if (this.commonService.getObjectId(refVoucher.Object, 'Code') != this.commonService.getObjectId(formGroup.get('Object').value)) {
                    this.commonService.showToast(this.commonService.translateText('Khách hàng trong phiếu báo giá không giống với phiếu bán hàng'), this.commonService.translateText('Common.warning'), { status: 'warning' });
                    continue;
                  }
                } else {
