...
import { SmartTableThumbnailComponent, SmartTableNumberEditableComponent, SmartTableCheckboxComponent, SmartTableCurrencyEditableComponent, SmartTableTextEditableComponent } from '../../../../lib/custom-element/smart-table/smart-table.component';
...
export class AccMasterBookFormComponent extends DataManagerFormComponent<AccMasterBookModel> implements OnInit {
  ...
  static _warehouseList: AccMasterBookEntryModel[];
  ...
  async loadCache() {
    ...
    // WarehouseBookFormComponent._goodsContainerList = WarehouseBookFormComponent._goodsContainerList || (await this.apiService.getPromise<AccMasterBookModel[]>('/warehouse/goods-containers', { includePath: true, includeIdText: true, includeWarehouse: true })).map(item => ({...item, 'text': item.Path})).sort((a, b) => a.Path.localeCompare(b.Path));
    return super.loadCache();
  }

  async loadGoodsContainerList(warehouseCode: string) {
    // WarehouseBookFormComponent._goodsContainerList = (await this.apiService.getPromise<AccMasterBookModel[]>('/warehouse/goods-containers', { includePath: true, includeIdText: true, filter_Warehouse: warehouseCode })).map(item => ({...item, 'text': item.Path})).sort((a, b) => a.Path.localeCompare(b.Path));
  ...
  async formLoad(formData: AccMasterBookModel[], formItemLoadCallback?: (index: number, newForm: FormGroup, formData: AccMasterBookModel) => void) {
  ...
  settings = this.configSetting({
    ...
    columns: {
      ...
      //     });
      //     instance.click.subscribe(async (row: AccountModel) => {
      //     });
      //   },
      // },
      Code: {
      ...
      Debit: {
        ...
        editable: true,
        delay: 3000,
        onChange: (value: number, row: AccountModel, instance: SmartTableNumberEditableComponent) => {
  ...
  /** Local dat source */
  source: CustomServerDataSource<AccountModel>;
  ...
  initDataSource() {
    this.source = new CustomServerDataSource<AccountModel>(this.apiService, '/accounting/accounts');

    // Set DataSource: prepareData
    this.source.prepareData = (data: AccountModel[]) => {
  ...
  loadList(callback?: (list: AccountModel[]) => void) {
    this.selectedIds = [];
    this.hasSelect = 'none';
    if (!this.source) {
      this.initDataSource();
  ...
  /** Edit event */
  onEditAction(event: { data: AccountModel }) {
    // this.router.navigate(['modules/manager/form', event.data[this.idKey]]);
    this.openProductForm([event.data['Code']]);
  }

  /** Implement required */
  openProductForm(ids?: string[], onDialogSave?: (newData: AccountModel[]) => void, onDialogClose?: () => void) {
    this.commonService.openDialog(ProductFormComponent, {
      context: {
        inputMode: 'dialog',
        inputId: ids,
        onDialogSave: (newData: AccountModel[]) => {
