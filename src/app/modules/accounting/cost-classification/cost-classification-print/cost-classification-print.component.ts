...
import { CashVoucherDetailModel, CostClassificationModel } from '../../../../models/accounting.model';
...
@Component({
  selector: 'ngx-cost-classification-print',
  templateUrl: './cost-classification-print.component.html',
  styleUrls: ['./cost-classification-print.component.scss']
})
export class AccCostClassificationPrintComponent extends DataManagerPrintComponent<CostClassificationModel> implements OnInit {

  /** Component name */
  componentName = 'AccCostClassificationPrintComponent';
  ...
  constructor(
    ...
    public ref: NbDialogRef<AccCostClassificationPrintComponent>,
  ...
  renderTitle(data: CostClassificationModel) {
  ...
  getTotal(data: CostClassificationModel) {
    let total = 0;
  ...
  saveAndClose(data: CostClassificationModel) {
  ...
  approvedConfirm(data: CostClassificationModel) {
    ...
    this.cms.showDialog(this.cms.translateText('Common.confirm'), this.cms.translateText(processMap?.confirmText, { object: this.cms.translateText('Sales.PriceReport.title', { definition: '', action: '' }) + ': `' + data.Description + '`' }), [
      ...
      },
      {
        ...
        action: () => {
          this.loading = true;
          this.apiService.putPromise<CostClassificationModel[]>(this.apiPath, params, [{ Code: data.Code }]).then(rs => {
            this.loading = false;
  ...
  async getFormData(ids: string[]) {
    return this.apiService.getPromise<CostClassificationModel[]>(this.apiPath, { id: ids, eq_Type: 'RECEIPT', includeContact: true, includeDetails: true, includeRelativeVouchers: true }).then(data => {
      this.summaryCalculate(data);
      return data;
    });
  }

  getItemDescription(item: CostClassificationModel) {
    return item?.Description;
  }

  summaryCalculate(data: CostClassificationModel[]) {
    for (const i in data) {
      const item = data[i];
      item['Total'] = 0;
