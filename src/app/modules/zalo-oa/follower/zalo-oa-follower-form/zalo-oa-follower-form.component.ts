...
@Component({
  selector: 'ngx-zalo-oa-follower-form',
  templateUrl: './zalo-oa-follower-form.component.html',
  styleUrls: ['./zalo-oa-follower-form.component.scss'],
})
export class ZaloOaFollowerFormComponent extends DataManagerFormComponent<ContactModel> implements OnInit {

  componentName: string = 'ZaloOaFollowerFormComponent';
  idKey = 'Code';
  baseFormUrl = '/zalo-oa/follower/form';
  ...
  select2GroupsOption = {
    ...
    ajax: {
      ...
      transport: (settings: JQueryAjaxSettings, success?: (data: any) => null, failure?: () => null) => {
        console.log(settings);
        const params = settings.data;
        this.apiService.getPromise('/contact/groups', { filter_Name: params['term'] }).then(rs => {
          success(rs);
        }).catch(err => {
          console.error(err);
          failure();
